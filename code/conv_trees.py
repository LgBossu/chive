...
#             logger.debug(f"Linked {parent_id} to {child_id}")
#         else:
#             logger.error(
...
#             for child_id in json.loads(node.metadata["children_ids"].replace("'", '"')):  # noqa: E501
...
#             #     graph.edge(neighbor.id, node.id)
#         graph.render(
...
# class ConversationsTree:
#     def __init__(self):
#         self.nodes: dict[str, MessageNode] = {}  # Store messages by ID

#     def add_message(self, message_id, content, metadata=None):
#         """Adds a new message node if it doesn't already exist."""
#         if message_id not in self.nodes:
#             self.nodes[message_id] = MessageNode(message_id, content, metadata)
...
#     def get_equivalence_class(self, message_id):
#         """Returns all connected messages as an 'equivalence class'."""
#         if message_id not in self.nodes:
#             return set()

#         visited, queue = set(), [self.nodes[message_id]]
#         while queue:
#             node = queue.pop()
#             if node not in visited:
#                 visited.add(node)
#                 queue.extend(node.get_neighbors())

#         return visited

#     def visualize(self):
#         """Placeholder for visualization logic (Graphviz, NetworkX, etc.)"""
#         pass  # Could implement graph plotting later

#     def __repr__(self):
#         return f"ConversationTree({len(self.nodes)} messages)"
